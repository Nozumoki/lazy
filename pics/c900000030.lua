...
function s.costfilter(c)
	return not c:IsType(TYPE_TOKEN) and (c:IsType(TYPE_NORMAL) or c:IsSetCard(0xF9F))
end
...
function s.filter(c,e,tp)
	return c:IsMonster() and c:HasLevel() and (c:IsType(TYPE_NORMAL) or c:IsSetCard(0xF9F))
...
function s.exfilter(c,sfunc)
	return c:IsSetCard(0xF9F) and sfunc(c)
end
