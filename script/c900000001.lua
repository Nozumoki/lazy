...
s.listed_names={id}
function s.filter(c)
	return c:IsSetCard(0x1908) and not c:IsCode(id) and c:IsSummonable(true,nil)
end
...
function s.thfilter(c)
	return c:IsSetCard(0x1908) and c:IsSpell() and not c:IsCode(id) and c:IsAbleToHand()
end
...
function s.lvfilter(c)
	return c:IsFaceup() and c:HasLevel() and c:GetLevel()~=4 and c:IsSetCard(0x1908)
end
