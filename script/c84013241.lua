--Rank-Up-Magic Radiant Star
local s,id=GetID()
function c84013241.initial_effect(c)
	--Activate
...
s.listed_series={0x107f,0x406}
function s.filter1(c,e,tp)
	local m=c:GetMetatable(true)
	local pg=aux.GetMustBeMaterialGroup(tp,Group.FromCards(c),tp,nil,nil,REASON_XYZ)
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsFaceup() and c:IsSetCard(0x107f) and m and (c:GetRank()>0 or c:IsStatus(STATUS_NO_LEVEL)) 
...
function s.filter2(c,e,tp,mc,rk,no,pg)
	...
	return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk)
		and c:IsSetCard(0x406) and c.xyz_number==no and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0
